...
指向本套件自身檔案的路徑相對於 `skills/claude/`，除非它以 repository 根層級
...
|----------|------------------------|--------|
| `plugins/codex/skills/*` | `skills/claude/`（一般 skill 目錄） | `codex-plugins/misty-claude/skills/*` |
| `plugins/codex/` 底下其餘全部 | `skills/claude/codex-plugin/`（overlay） | `codex-plugins/misty-claude/`（plugin root） |
