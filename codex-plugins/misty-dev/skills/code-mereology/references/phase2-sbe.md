...
- Every test file MUST name its leaf in a header comment, using that language's comment syntax: `code-mereology-leaf: <repository-relative-path>`. Every file in one Phase 2 set MUST carry the identical path, and it MUST resolve to a confirmed design document — never to a `-perf` record or `-plan` file. MUST NOT infer the owner from what sits near the code — that reading breaks silently as soon as a folder gains a second `sd-*.md`.
