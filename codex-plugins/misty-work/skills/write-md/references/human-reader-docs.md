...
- Before finalizing a diagram, MUST render it with the target renderer, or the same Mermaid version, when one is available, and confirm that the expected nodes, edges, messages, or slices appear without a parser error. If only a different compatible renderer is available, MUST use it to catch parser errors and state that target rendering remains unverified. Markdown lint, fence checks, link checks, and manual inspection are not render validation. If no compatible renderer is available, MUST review every rule in this section and state that rendering was not verified. MUST NOT present those checks as proof that the diagram renders.
