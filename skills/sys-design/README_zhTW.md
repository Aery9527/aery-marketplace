...
一套只寫設計文件的開發流程，再透過 Specification by Example 與 TDD 把設計變成程式碼。
...
| `sd-<功能>-perf.md` | 與它所量測的程式碼放在一起 | 一次次累積下來的效能量測結果 |
...
**由使用者刪減測試，而不是勾選清單。** 早期版本要 agent 把 edge case 列成清單讓使用者逐項標記。實際上判斷具體的 input 與 output 比判斷對它們的抽象描述更容易，所以拿來檢視的是測試本身。
