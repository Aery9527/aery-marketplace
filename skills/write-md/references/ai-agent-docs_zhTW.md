...
- 不要求 `[返回開頭]` 連結。
- 不使用 Mermaid。
- 禁止使用 Markdown table；改用條列清單或編號步驟。
- 不讀取 [references/diagram-examples_zhTW.md](diagram-examples_zhTW.md)，除非使用者明確要求在 AI agent 文件中包含 Mermaid 語法說明。
...
- 對條件分支使用「若 ... 則 ...；否則 ...」格式。
- 對多選一決策使用優先序條列或「若 X 則 Y；否則 Z」語氣；禁止使用 table。
